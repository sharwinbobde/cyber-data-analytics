...
def main(config_file="parameters.ini"):
    ...
    assert (
        eval(parameters["dataset"]["generate_feature_vector_files"]) is False
    ), assertion_message
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=float(parameters["hyperparam"]["ff_learning_rate"])
    )
    ...
    def train(epoch):
        ...
        for batch_idx, ((bon_x, bon_y), (mal_x, mal_y)) in enumerate(
            ...
            if batch_idx > 1000:
                break
            bon.append(bon_x.cpu().numpy())
            mal.append(mal_x.cpu().numpy())
            ...
            if batch_idx % log_interval == 0:
                ...
                current_time = time.time()
                print(
                    "Train Epoch ({}) | Batch ({}) | [{}/{} ({:.0f}%)]\tBatch Loss: {:.6f}\tBatch Accuracy: {:.1f}%\t BSCN: {:.12f}".format(
                        epoch,
                        batch_idx,
                        batch_idx * len(x),
                        len(train_dataloader_dict["malicious"].dataset)
                        + len(train_dataloader_dict["benign"].dataset),
                        100.0 * batch_idx / len(train_dataloader_dict["benign"]),
                        loss.data[0],
                        100.0 * correct / len(y),
                        bscn.ratio(),
                    )
        ...
        mal = np.vstack(mal)
        adv = np.vstack(adv)
        np.save("benign.npy", bon)
        np.save("malicious.npy", mal)
        np.save("adversarial.npy", adv)
    ...
    def check_one_category(
        category="benign", dset_type="test", is_evade=False, evade_method="dfgsm_k"
    ):
        """
    ...
    def test(epoch, dset_type="test"):
        ...
        (
            evade_mal_total_loss,
            evade_mal_total_correct,
            evade_mal_total,
        ) = check_one_category(
            category="malicious",
            is_evade=True,
        ...
        print(
            "{} set overall: Average Loss: {:.4f}, Accuracy: {:.2f}%".format(
                dset_type, total_loss / total, total_correct * 100.0 / total
            )
        )
        ...
        return (
            (bon_total_loss + max(mal_total_loss, evade_mal_total_loss)) / total,
            metrics,
        )
    ...
    def process_adv_examples(evade_method="dfgsm_k", mode="gen"):
        ...
        for dset_type in ["train", "val", "test"]:
            ...
            for batch_idx in range(num_batches):
                ...
                assert np.allclose(
                    np.logical_and(_x, _x_adv), _x
                ), "perturbation constraint violated"
                if mode == "gen":
        ...
        metrics = {
            ...
            "bon": {"total_loss": 1, "total_correct": 1, "total": 1},
        }
    ...
    if not eval(parameters["challenge"]["eval"]) and eval(
        ...
        for _epoch in range(num_epochs):
            ...
            if best_valid_loss > valid_loss:
                ...
                with open(
                    os.path.join("result_files", "%s_bscn.txt" % experiment_name), "w"
                ) as f:
                    f.write(str(bscn_to_save))
                torch.save(
                    model, os.path.join("helper_files", "%s-model.pt" % experiment_name)
                )
    ...
    with open(
        os.path.join("result_files", experiment_name + ".json"), "w"
    ) as result_file:
